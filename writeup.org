...
Currently, no proxy of policy alignment measures Internet governance. Internet
...
at the protcol layer, where a transition from IPv4 to IPv6 plays out unevenly
around the globe). Future work could also explore fragmentation "higher up" the
stack, looking at laws that limit data flow or enforce Internet sovreignty
cite:Bejtlich2015.

#+caption: A "layer" model of the Internet, with possible proxies for each layer. The work we present here only covers Layer 4.
#+label: tab:layers
...
as possible (and archiving prior metrics). The [[https://ooni.org/][Open Observatory of Network
Interference]] (OONI) collects Internet measurements continuously, including data
...
that violate US law. One prolific effort is [[https://www.ice.gov/factsheets/ipr-in-our-sites][Operation In Our Sites]], managed by
Immigration and Customs Enforcement (ICE). While this activity does constitute
Internet censorship, it does not cause Internet fragmentation /per se/, as
seized websites become equally unavailable worldwide. Future work should monitor
US website seizing operations more closely.
