...
#+HTML_HEAD: <script type="text/javascript" src="index.bundle.js"></script>
...
formal trade agreements (e.g., EEA) and formal alliances (e.g., NATO) only
capture certain aspects of states’ alignment, researchers have looked for other
measures of policy alignment to fill in the gaps.

This work introduces a new proxy of policy alignment: website blocking. This
proxy is particularly interesting because it speaks to the information flow in
the digital layer of international politics, a new element to international
trade and cooperation, one which existing proxy metrics fail to capture a
priori. In addition, our proxy measure can be constantly scraped and updated,
offering a higher temporal resolution than existing proxies.

Big ideas:
- Our work suggests a link between Internet governance and other issues in international relations
- Since we can capture our metric in real time, we can use changes in our proxy
  to detect possible changes in the IR landscape faster than would otherwise be
  possible
# - we are suggesting our proxy for application to your domain of interest
...
In this project, we're trying to use similarities in webpage blocking patterns
to achieve a similar goal. If two countries block the "same" thing (more on that
in a minute), that's a proxy of policy alignment. They may not have conferred,
but their blocking decision reveals a shared outlook or set of geopolitical
goals.

#+CAPTION: Different proxies of policy alignment create connected graphs of countries. We can compare these proxies across topic areas. Our project adds a new topic area of the Internet.
#+LABEL: fig:proxies
[[./img/proxies.png]]

Compared to existing proxies, ours captures a new domain: Internet governance.
...
blocking is typically perpetrated directly by governments. From a technical
standpoint, there are a variety of ways to block webpages. One option is to
manipulate domain name system (DNS) messages in order to make websites appear
unresolveable. Another option is to drop TCP handshake packages with particular
destination IPs, effectively making IP addresses impossible to contact. A
related option is to allow the TCP handshake to occur, but inject follow-up TCP
packets that either break or supersede the legitimate response. Yet another
option is to make a legal case to a particular service provider (for example,
asking Facebook to block a page in a particular country).
...
* Methods
Before each of these experiments, we cleaned and pre-processed the data. See
...
This metric reveals a few patterns we would expect to see. Countries with little
to no blocking (Israel, Iceland, USA, Finland) lie close together.

However, the metric also reveals a few surprises. Venezuela has no website
blocking despite a deteriorating human rights situation during the period these
observations were collected []; it sits with other countries typically
considered “free” or “open” in their approach to Internet governance (and, in
popular imagination, free speech by extension).

Another surprise: our metric also captures significant differences between
countries typically considered to have “closed” internets. Saudi Arabia has a
unique approach to blocking from any other “closed” Internet: it heavily
censored material it finds morally objectionable (e.g., pornography). In
contrast, India and Turkey block a mix of religious and political content, and
the particular weights of each give them some distance from one another. 
# TODO check this.

China, which is popularly imagined to export its Internet to its Belt & Road
allies, in fact has low similarity with such countries. It is relatively unique
(no more than 60% similar to any other country), but is most similar to
countries with which it has no close ties, such as Peru, France and Netherlands.
# TODO How are they similar? Along what dimensions?

Finally, our findings reveal a number of countries - the majority of our
dataset - somewhere in the middle; relatively similar in blocking websites that
are content similarity descriptor. A few thoughts on what this means.

# A few general thoughts on the trends and patterns this analysis reveals; what we
# learn about supposed allies, etc etc.

The value of our findings can be, as suggested earlier, dual. First, our work
...
work could provide a measure of cross-national information flows and Internet
governance. The following section turns to this question, comparing our metric
to existing measurements of policy and asking to what degree our measurement is
distinctive, and to what degree it overlaps with traditional domains of
...
# LATER - summarize findings
# LATER - highlight ‘leading indicator’ concept
...
Future work could use our proxy as a “leading indicator” of changes in trade
relations, as we can measure Internet blocking continuously. As a country’s
blocking similarity becomes more similar to a given set of countries, we would
expect those countries to be more likely to join a trade pact in the future.
  
Future work could also explore aspects of global trade outside of formal
agreements. Non-tariff barriers to trade [Simon J Evenett] present one potential
...
** Cultural Affinity: Commonwealth Countries
<<commonwealth>>
...
* WAIT Conclusion                                                   :ARCHIVE:
- what we did, what it could be used for
- add what-if scenarios
- leading indicator; find big changes in real-time internet blocking patterns, use them to predict changes in other domains
  for example, if the US blocks wechat and europe does not, then we could predict that europe will be trading more wtih china than the US will.

# use these points for fundraising in the future
#  raise funds to maket his into a continuous 'service'; type of intelligence.
#   pay us to develop this into something continuous
